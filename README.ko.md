...
Claude Code 안에서 아래 명령어를 순서대로 입력하세요:
```
...
또는 레포를 클론한 뒤 `.cursor-plugin/` 디렉토리를 Cursor에 연결합니다.
...
자세한 내용은 [Codex 설치 가이드](.codex/INSTALL.md)를 참조하세요.
...
2. AI와 인증 방식 브레인스토밍
   ...
   -> JWT 선택
   -> AI가 결정 기록 (플랜 모드면 지연 기록)

3. 구현 계획 수립
   -> 계획 수정: "단기 토큰만이 아니라 리프레시 토큰도 사용"
   -> AI가 계획 수정 결정을 기록 (플랜 파일에 지연 기록)

4. 계획 종료, 코딩 시작
   -> AI가 지연된 결정들을 docs/decisions/*.md로 변환
...
| | 브레인스토밍 | Why Log |
|---|---|---|
| **시점** | 코딩 **전** — 뭘 만들지 설계 | **전 과정** — 요청부터 최종 결과까지 |
| **산출물** | 디자인 스펙 (`docs/superpowers/specs/`) | 결정 로그 (`docs/decisions/`) |
| **트리거** | 사용자가 직접 시작 | 완전 자동 — 감지해서 알아서 기록 |
| **독자** | 나와 팀 (기획 단계) | PR 리뷰어와 미래의 나 |
| **기록 대상** | 설계 프로세스 | 선택의 이유 |

**실제로는 같이 씁니다:**

1. "인증 어떻게 만들지?" 브레인스토밍 → JWT vs 세션 선택 → **why-log가 기록**
2. 구현 계획 수립 → 계획 수정 → **why-log가 변경 사항 기록**
3. 구현 시작 → 계획과 다르게 변경 → **why-log가 결정 로그 업데이트**
4. 작업 완료 → **why-log가 최종 결과 기록**
5. PR 생성 → 전체 의사결정 여정이 PR 본문에 자동 포함

브레인스토밍은 *설계 프로세스*입니다. Why Log는 *모든 선택의 이유*를 기록합니다 — 초기 요청부터 계획, 구현, 최종 결과까지.
